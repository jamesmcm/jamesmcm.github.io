...
AWS events with one Lambda function (via Serde), parse received emails
...
Note that in [a previous post](http://jamesmcm.github.io/blog/2020/04/19/data-engineering-with-rust-and-aws-lambda/#en) I dealt with create a Lambda based
pipeline for parsing Excel files and writing to Redshift. There are a
few differences and things to note in this process:

* We will use Rustls not OpenSSL (so it is easier to statically link
  with musl) - in this case, we only need to select that feature in
  Rusoto.
* The Rusoto API has changed to be asynchronous, so we must instantiate
  a Tokio runtime (i.e. we cannot just use `.sync()` anymore)
* I am still using the crates.io version of the lambda_runtime crate, so
  we do not have complete async support (the Github master branch _does_
  allow you to just use async functions directly!)
* Here we will deal with parsing email MIME-formatted messages.

Note that the justification for using Rust is the same - mainly that we
...
We can break this down in to the following steps:
...
don't have one). You can also specify specific recipients for the rule.
...
via GMail (in reality, there are a load of DKIM headers, etc. prior to
...
with a separator `--000000000000d2fb5805ac63efb8` in the above case
(note this is specified where the multipart begins).

In the above case we can see the email
contains both a plaintext and HTML version. This means the body is
...
* A multipart for the actual body, this is an alternative multipart so
...
So the client will choose either the HTML or plaintext body, and always
...
extract the attachment body and attachment filename for the _first_
attachment:
...
This gives us the attachment as a `String`. Note if we were not dealing
...
* A positive ID number: `u32`
* A start date
* An end date

We will use the [chrono crate](https://crates.io/crates/chrono) to deserialise the dates,
and the [csv crate](https://crates.io/crates/csv) with [Serde](https://crates.io/crates/serde)
to do the overall deserialisation.
...
Note that by default SES will be in [Sandbox mode](https://docs.aws.amazon.com/ses/latest/DeveloperGuide/request-production-access.html). This means you must
...
Then we make the request with Rusoto, note we need to block on the
Future returned from `send_raw_email()`:
...
crate for deserialising the events we receive. In this case we want to
...
Then we can deserialise to `EventEnum` in our handler function:
...
Now we can compile to musl (here I assume a GNU/Linux platform,
...
Note we could probably also parse the received emails with lettre
instead of [mailparse](https://crates.io/crates/mailparse). But I found
mailparse so easy to use, I wanted to highlight it here so others will
use it too.
