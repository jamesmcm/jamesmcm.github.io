...
layout: post
title: "My Rust 2021"
titleen: "My Rust 2021"
titlesp: "My Rust 2021"
...
with Rust, the current strengths and weaknesses and areas I'd like to
...
the hard parts of this are largely taken care of by other teams).
...
blog posts on Rust, the most popular being [about data-oriented
programming](http://jamesmcm.github.io/blog/2020/07/25/intro-dod/#en) and [an introduction to async programming](http://jamesmcm.github.io/blog/2020/05/06/a-practical-introduction-to-async-programming-in-rust/#en).
...
the majority of use cases. For Rust 2021 this might include
...
class of bugs and also aren't bound by issues like the [Global
Interpreter Lock](https://en.wikipedia.org/wiki/Global_interpreter_lock) or the lack of type safety as in simpler scripting
languages.
...
Myself, I struggled greatly with Serde's `deserialize_with` attribute until looking at examples.
...
fn main() {
    ...
    match myenum {
        MyEnum::WithString(x) if x == "test" => {println!("Was string")},
        _ => {},
    }
}
```
...
defaulting to `'static`. Recently, in this [PR to the dialoguer
crate](https://github.com/mitsuhiko/dialoguer/pull/81/files#diff-25c234b5525af56f20a73cb80796026bR47).
...
lifetime then the trait object would default to that lifetime. Perhaps
