...
real useful Rust project (I suppose [s3rename](https://github.com/jamesmcm/s3rename) was also useful, but a much smaller
scope).
...
This is problematic for checking whether the user-entered private key
...
final application has terminated. So if lockfiles still exist, we need
to prevent the clean-up destructors from firing for the vopono instances
terminated first.
...
trait object to subtrait. That is we want something like:
...
But no such function exists. As far as I can tell, the [downcast-rs](https://crates.io/crates/downcast-rs)
...
the trait object we want](https://github.com/jamesmcm/vopono/blob/master/src/providers/mod.rs#L42). But this is a painful workaround as we have
a fair amount of boilerplate code and some extra allocation of trait objects.
...
to generated .deb packages with [cargo-deb](https://github.com/mmstick/cargo-deb),
...
the issues mentioned above for a start, and there are [always more VPN
providers to support!](https://github.com/jamesmcm/vopono/issues/10)
